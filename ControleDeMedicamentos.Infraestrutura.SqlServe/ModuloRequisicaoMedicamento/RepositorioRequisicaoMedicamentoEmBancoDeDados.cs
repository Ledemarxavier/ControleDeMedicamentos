...
namespace ControleDeMedicamentos.Infraestrutura.BancoDeDados.ModuloRequisicaoMedicamento;

public class RepositorioRequisicaoMedicamentoEmBancoDeDados(IDbConnection connection)
{
    ...
    public RequisicaoEntrada? SelecionarRequisicaoEntradaPorId(Guid id)
    {
        ...
        INNER JOIN [TBMedicamento] m ON m.[Id] = e.[MedicamentoId]
        LEFT JOIN [TBFuncionario] fun ON fun.[Id] = e.[FuncionarioId]
        WHERE e.[Id] = @Id;
    ";

        var requisicaoEntrada = connection.Query<RequisicaoEntrada, Medicamento, Funcionario, RequisicaoEntrada>(
            sql,
            map: (requisicao, medicamento, funcionario) =>
            {
            ...
            param: new { Id = id },
            splitOn: "MedicamentoId,FuncionarioId"
        ).FirstOrDefault();
    ...
    public RequisicaoSaida? SelecionarRequisicaoSaidaPorId(Guid id)
    {
        ...
        SELECT
            ...
            -- Prescrição
            p.[Id] AS Split_PrescricaoId,
            p.[Descricao] AS PrescricaoDescricao,

            -- Funcionário
            fun.[Id] AS Split_FuncionarioId,
            fun.[Nome] AS FuncionarioNome,

            -- Paciente
            pac.[Id] AS Split_PacienteId,
            pac.[Nome] AS PacienteNome
        FROM [TBRequisicaoSaida] rs
        LEFT JOIN [TBPrescricao] p ON p.[Id] = rs.[PrescricaoId]
        LEFT JOIN [TBPaciente] pac ON pac.[Id] = p.[PacienteId]
        LEFT JOIN [TBFuncionario] fun ON fun.[Id] = rs.[FuncionarioId]
        WHERE rs.[Id] = @Id;
    ";

        var requisicaoSaida = connection.Query<RequisicaoSaida, Prescricao, Funcionario, RequisicaoSaida>(
            sql,
            map: (requisicao, prescricao, funcionario) =>
            {
                requisicao.Prescricao = prescricao;
                requisicao.Funcionario = funcionario;

                if (prescricao != null && prescricao.Id != Guid.Empty)
                {
                    prescricao.MedicamentosPrescritos = CarregarMedicamentosPrescritos(prescricao.Id);

                    prescricao.Paciente = new Paciente
                    {
                        Id = connection.ExecuteScalar<Guid>(
                            "SELECT PacienteId FROM TBPrescricao WHERE Id = @Id",
                            new { Id = prescricao.Id }),
                        Nome = connection.ExecuteScalar<string>(
                            @"SELECT pac.Nome 
                          FROM TBPaciente pac 
                          INNER JOIN TBPrescricao p ON p.PacienteId = pac.Id 
                          WHERE p.Id = @Id",
                            new { Id = prescricao.Id })
                    };
                }
            ...
            param: new { Id = id },
            splitOn: "Split_PrescricaoId,Split_FuncionarioId"
        ).FirstOrDefault();
    ...
    private List<MedicamentoPrescrito> CarregarMedicamentosPrescritos(Guid prescricaoId)
    {
        const string sql = @"
        SELECT
    mp.[MedicamentoId],
...
WHERE mp.[PrescricaoId] = @PrescricaoId;
    ";
        ...
        var medicamentosPrescritos = connection.Query<MedicamentoPrescrito, Medicamento, MedicamentoPrescrito>(
            sql,
            map: (medPrescrito, medicamento) =>
            {
                medPrescrito.Medicamento = medicamento;
                return medPrescrito;
            },
            param: new { PrescricaoId = prescricaoId },
            splitOn: "Id"
        ).ToList();
